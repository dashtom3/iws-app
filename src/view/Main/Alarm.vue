<template>
    <div class="alarm">
        ...
        <mu-card v-for="item,index in alarmList">
          ...
          <mu-card-header :title="'设备名称:'+item.groupName" :subTitle="'房间:'+item.roomName">
          </mu-card-header>
          <!-- <mu-card-title title="Content Title" subTitle="Content Title"/> -->
          <div class="alarm-text">
          <span v-if="item.show == null || item.show == false">
            <b>报警日期:&nbsp;&nbsp;{{item.startTime | dateAll}}-{{item.endTime | dateAll}}</b><br/>
          ...
          <mu-card-actions>
            <mu-flat-button v-if="item.confirm == 1" icon="done" label="已确认" primary/>
            <mu-flat-button v-if="item.confirm == 0" icon="search" label="未确认" primary/>
          </mu-card-actions>
        </mu-card>
        <mu-raised-button label="加载更多" @click="loadMore" default fullWidth v-if="alarmList != null && args.totalPage != args.currentPage"/>
        <mu-raised-button label="没有更多内容了" default fullWidth v-if="alarmList != null && args.totalPage == args.currentPage"/>
...
export default{
  ...
  methods: {
    ...
    getAlarmList() {
      ...
      this.$store.dispatch(type.ALARM_LIST, this.args).then((data) => {
        if (self.alarmList == null) {
          ...
          self.alarmList = self.alarmList.concat(data.data);
        }
    ...
    showMore(item, index) {
      item.show = !item.show;
      this.$set(this.alarmList, index, item);
